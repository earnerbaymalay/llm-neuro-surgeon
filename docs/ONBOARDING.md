...
Run `scan` first. It's read-only and takes seconds. `import --dry-run` shows you exactly what will move into the Brain before anything happens. Only then run `import` for real.
...
- **Auto-Sync** — `synapse sync` starts the watcher and keeps resolving drift with a 3-way merge as it happens; `synapse sync --once` runs a single pass and exits, useful for scripts and CI.
