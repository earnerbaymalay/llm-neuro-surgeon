...
impl Adapter for GitHubCopilotAdapter {
    ...
    fn import(&self, root: &Path) -> Result<ImportResult, AdapterError> {
        ...
        if dot_github_path.exists() {
            let raw = fs::read_to_string(&dot_github_path).map_err(|e| {
                AdapterError::Io(format!(
                    "Failed to read .github/copilot-instructions.md: {}",
                    e
                ))
            })?;
        ...
        if root_instructions_path.exists() {
            let raw = fs::read_to_string(&root_instructions_path).map_err(|e| {
                AdapterError::Io(format!("Failed to read copilot-instructions.md: {}", e))
            })?;
            let content = strip_provenance(&raw);
            let sha256 = compute_sha256(&content);
            skills.push(Skill {
        ...
        if mcp_path.exists() {
            ...
            let parsed: Value = serde_json::from_str(&cleaned).map_err(|e| {
                AdapterError::Malformed(format!("Invalid JSON in .vscode/mcp.json: {}", e))
            })?;

            if let Some(mcp_servers_val) = parsed.get("mcpServers").and_then(|v| v.as_object()) {
                for (id, val) in mcp_servers_val {
                    parse_and_validate_mcp_server(id, val, "github-copilot", &mut mcp_servers)?;
                }
            }
        }
    ...
    fn project(
            ...
            if let Some(trigger) = skill.triggers.first() {
                ...
                if let Some(parent) = target_path.parent() {
                    fs::create_dir_all(parent).map_err(|e| {
                        AdapterError::Io(format!(
                            "Failed to create parent directory for scoped skill: {}",
                            e
                        ))
                    })?;
                }
                ...
                fs::write(&target_path, output).map_err(|e| {
                    AdapterError::Io(format!(
                        "Failed to write scoped skill to {}: {}",
                        relative_target, e
                    ))
                })?;
        ...
        if !copilot_servers.is_empty() {
            ...
            fs::create_dir_all(&mcp_dir).map_err(|e| {
                AdapterError::Io(format!("Failed to create .vscode directory: {}", e))
            })?;
            ...
            let mut current_json = if mcp_path.exists() {
                let raw_json = fs::read_to_string(&mcp_path).map_err(|e| {
                    AdapterError::Io(format!("Failed to read .vscode/mcp.json: {}", e))
                })?;
                let cleaned = clean_jsonc(&raw_json);
            ...
            fs::write(&mcp_path, pretty).map_err(|e| {
                AdapterError::Io(format!("Failed to write .vscode/mcp.json: {}", e))
            })?;
...
mod tests {
    ...
    fn test_github_copilot_import_export_roundtrip() {
        ...
        fs::write(
            ...
            r#"{"mcpServers": {"test-mcp": {"command": "npm", "args": ["run", "mcp"]}}}"#,
        )
        .unwrap();
        ...
        let global_skill = imported
            .skills
            .iter()
            .find(|s| s.id == "github-copilot-instructions")
            .unwrap();
        ...
        assert!(project_res
            .written
            .contains(&"src/rust.instructions.md".to_string()));
        assert!(project_res
            .written
            .contains(&".vscode/mcp.json".to_string()));

        let projected_global =
            fs::read_to_string(out_dir.path().join(".github/copilot-instructions.md")).unwrap();
        assert!(projected_global.contains("global config"));

        let projected_scoped =
            fs::read_to_string(out_dir.path().join("src/rust.instructions.md")).unwrap();
