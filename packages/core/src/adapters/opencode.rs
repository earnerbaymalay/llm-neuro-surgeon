...
fn parse_yaml_frontmatter(fm: &str) -> (Vec<String>, Vec<String>, Vec<String>) {
    ...
    for line in fm.lines() {
        ...
        if line_trimmed.starts_with('-') {
            let val = line_trimmed[1..].trim().trim_matches('"').trim_matches('\'').to_string();
...
fn serialize_yaml_frontmatter(tools: &[String], model_hints: &[String], targets: &[String]) -> String {
    ...
    fn project(
        ...
        if opencode_agent.is_some() || !opencode_skills.is_empty() {
            let (tools, model_hints, targets) = if let Some(agent) = opencode_agent {
                (agent.tools.clone(), agent.model_hints.clone(), agent.targets.clone())
            } else {
...
over multiple lines."#;
        ...
        assert_eq!(imported.skills[0].source, "Some system instructions\nover multiple lines.");
        ...
        assert!(projected_content.contains("<!-- GENERATED BY LLM NEUROSURGEON — edit in the Brain -->"));
